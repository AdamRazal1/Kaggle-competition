...
print(len(train_data.values) * 0.01)

id = train_data['Id']

# Dropping ambiguous features

train_data.drop(['MiscFeature', 'MiscVal', 'Id', 'PoolQC', 'Fence', 'Alley', 'Utilities', 'Condition2', 'HouseStyle', 'RoofMatl', 'Exterior1st', 'Exterior2nd', 'Heating', 'Electrical', 'GarageQual'], inplace=True, axis=1)
...
encoded_categorical_test = encoder.fit_transform(test_data[categorical_features])
encoded_categorical_test = pd.DataFrame(encoded_categorical_test, columns=encoder.get_feature_names_out(categorical_features))

new_numerical_features = [col for col in numerical_features if col != 'SalePrice']
...
new_combined_features_test = new_categorical_features + new_numerical_features
...
scaled_numerical_test = scaled_numerical.copy()
...
print('This is combined data', combined_data.columns)

combined_data_test = pd.concat([encoded_categorical_test, scaled_numerical_test], axis = 1)
print('This is combined data for testing', combined_data_test.columns)

# Storing cleaned data into a csv file
# combined_data_test.to_csv('cleaned_test.csv')
...
X_test = combined_data_test[[col for col in new_combined_features_test if col != 'SalePrice']]
 
selector = SelectKBest(score_func=f_classif, k=274)
...
submission = pd.DataFrame({
    'Id': range(1461, 1461 + len(test_data)),
